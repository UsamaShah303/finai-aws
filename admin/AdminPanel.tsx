      ...
      >
        <div className="p-10 flex items-center gap-5 relative">
          ...
          {!isCollapsed && (
            ...
            >
               <span className={`text-2xl font-black tracking-tighter ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>FinAI <span className="text-indigo-500">Nexus</span></span>
               <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] leading-none mt-1">Core Admin</span>
            </motion.div>
          )}
          ...
          <button 
            ...
            className={`absolute -right-4 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full border shadow-xl flex items-center justify-center transition-all z-[60] hover:scale-110 active:scale-95 ${
              isDarkMode ? 'bg-gray-800 border-white/10 text-gray-300' : 'bg-white border-gray-200 text-gray-500'
            }`}
          >
        ...
        <nav className="flex-1 px-5 mt-10 space-y-3 overflow-y-auto scrollbar-hide">
          <div className="px-4 mb-4">
             <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em]">Main Modules</span>
          </div>
            ...
            <button
              ...
              className={`w-full flex items-center gap-4 p-4 rounded-[24px] transition-all relative group ${
                activeAdminTab === item.label 
                  ...
                  : `text-gray-400 hover:text-indigo-500 ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-gray-50'}`
        ...
        <div className="p-6 mt-auto border-t border-gray-100 dark:border-white/5">
           <button 
             ...
             className={`w-full flex items-center justify-center gap-3 p-5 rounded-[28px] border transition-all hover:shadow-lg ${
               isDarkMode ? 'border-white/5 text-gray-400 hover:bg-white/5' : 'border-gray-100 text-gray-500 hover:bg-gray-50'
             }`}
           >
        ...
        <header className={`h-28 px-12 flex items-center justify-between border-b transition-all duration-700 z-40 ${
          ...
          <div className="flex items-center gap-10 flex-1">
             <div className="flex items-center gap-2">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Nodes</span>
             ...
             <div className={`relative max-w-xl w-full group ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                ...
                <input 
                  ...
                  className={`w-full pl-14 pr-6 py-4 border-none rounded-[24px] text-xs font-bold transition-all outline-none ring-2 ring-transparent focus:ring-indigo-500/20 ${
                    isDarkMode ? 'bg-white/5 focus:bg-white/10 text-white' : 'bg-gray-50 focus:bg-gray-100 text-gray-900'
                  }`}
                />
          ...
          <div className="flex items-center gap-8">
             <div className="flex items-center gap-2">
                ...
                <button className={`w-12 h-12 rounded-[18px] flex items-center justify-center border transition-all relative hover:scale-105 active:scale-95 ${
                    isDarkMode ? 'border-white/10 text-gray-300 hover:bg-white/10 bg-white/5' : 'border-gray-100 text-gray-500 hover:bg-gray-50 bg-white shadow-sm'
                  }`}>
             ...
             <button className={`flex items-center gap-4 p-2 rounded-[22px] transition-all group border ${
                ...
                {!isCollapsed && (
                  <div className="flex flex-col items-start leading-none pr-4">
                    <span className={`text-sm font-black tracking-tight ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Admin User</span>
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">L7 Superuser</span>
                  </div>
                )}
